<?php
/**
 * The template for displaying the home page
 */
get_header(); ?>

<div id="primary" class="content-area">
    <main id="main" class="site-main" role="main">
        <div class="ev-header-img">
            <div>
                <div class="col-2"></div>
                <div class="col-8">
                    <img src="http://s30.postimg.org/m0x731825/spoon_lwhite.png">
                    <br>
                    <h1 class="top">WE FUND INNOVATIVE AND<br>DISRUPTIVE FOOD COMPANIES</h1>

                    <!-- Text that displays at the top of home page -->
                    <?php
                    $description = get_bloginfo( 'description', 'display' );
                    if ( $description || is_customize_preview() ) : ?>
                        <h1 class="site-description"><?php echo $description;?></h1>
                        <?php
                    endif; ?>
                    <br>
                    <br>
                    <a href="#intro"><i class="fa fa-arrow-down fa-2x"></i></a>
                </div>
                <div class="col-2"></div>
            </div>
        </div>

        <div id="intro" class="container-fluid white second">
            <div class="wrapper">
                <div class="row-fluid">
                    <div class="col-2"></div>
                    <div class="col-8">
                        <h2 class="padding">Our Process</h2></div>
                    <div class="col-2"></div>
                </div>
                <!-- row -->
                <div class="row-fluid">
                    <div class="col-4 spacer">
                        <img src="" class="icon">
                        <br>
                        <p><img src="http://s9.postimg.org/s5sdx8qob/rc_r1.png" style="width:40%;"></p>
                        <br>
                        <p class="bold">Connect</p>
                        <p class="small">Maecenas magna dui, dignissim quis felis sed, tincidunt molestie magna. Proin semper felis in dolor.</p>
                    </div>
                    <div class="col-4 spacer">
                        <img src="" class="icon">
                        <br>
                        <p><img src="http://s9.postimg.org/oxnwk74ej/rc_r2.png" style="width:40%;"></p>
                        <br>
                        <p class="bold">Scouting</p>
                        <p class="small">Maecenas magna dui, dignissim quis felis sed, tincidunt molestie magna. Proin semper felis in dolor.</p>
                    </div>
                    <div class="col-4 spacer">
                        <img src="" class="icon">
                        <br>
                        <p><img src="http://s9.postimg.org/9eqgn2w3z/rc_r3.png" style="width:54%;"></p>
                        <br>
                        <p class="bold">Presentations</p>
                        <p class="small">Maecenas magna dui, dignissim quis felis sed, tincidunt molestie magna. Proin semper felis in dolor.</p>
                    </div>
                </div>
                <!-- row -->
                <div class="row-fluid">
                    <div class="col-4"></div>
                    <div class="col-4">
                        <div class="bttn ev-red center">Tell me more&nbsp;&nbsp;<i class="fa fa-arrow-right"></i></div>
                    </div>
                    <div class="col-4"></div>
                    <!-- row -->
                </div>
                <!-- container -->
            </div>
        </div>

        <div class="container-fluid ev-red third">
            <div class="wrapper">
                <div class="row-fluid">
                    <div class="col-2"></div>
                    <div class="col-8"><img src="http://s30.postimg.org/m0x731825/spoon_lwhite.png" style="padding-bottom:1.5rem;">
                        <br>
                        <br>
                        <h2 class="bump">Meet our members</h2>
                        <br>
                        <p class="redtext bump">Made up exclusively of angels with domain expertise in creating,
                            <br>manufacturing distributing and selling food and beverage products</p>
                        <br>
                        <br>
                        <div class="btn btnred up">Meet our members&nbsp;&nbsp;<i class="fa fa-arrow-right"></i></div>
                        <br>
                        <br>
                    </div>
                    <div class="col-2"></div>
                </div>
                <!-- row -->
            </div>
        </div>
    </main><!-- #main -->
</div><!-- #primary -->

<?php get_footer();
